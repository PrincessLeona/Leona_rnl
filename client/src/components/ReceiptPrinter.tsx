...
const ReceiptPrinter: React.FC<ReceiptPrinterProps> = ({ transaction, onClose }) => {
    ...
    <div className="modal-overlay">
      <div className="modal" style={{ maxWidth: '600px' }}>
        <div className="modal-header">
          <h3 className="modal-title">Receipt - {transaction.transaction_number}</h3>
        ...
        <div className="modal-body">
          <div ref={receiptRef} className="receipt" style={{
            ...
            <div className="transaction-info" style={{
              ...
              <div><strong>Cashier:</strong> {transaction.user.first_name} {transaction.user.last_name}</div>
              {transaction.customer_name && (
                <div><strong>Customer:</strong> {transaction.customer_name}</div>
              )}
              {transaction.customer_email && (
                <div><strong>Email:</strong> {transaction.customer_email}</div>
              )}
            </div>
            ...
            <table className="items-table" style={{
              ...
              <thead>
                <tr>
                  <th style={{ borderBottom: '1px solid #000', fontSize: '10px', fontWeight: 'bold' }}>
                    Item
                  </th>
                  <th style={{ borderBottom: '1px solid #000', fontSize: '10px', fontWeight: 'bold', textAlign: 'center' }}>
                    Qty
                  </th>
                  <th style={{ borderBottom: '1px solid #000', fontSize: '10px', fontWeight: 'bold', textAlign: 'right' }}>
                    Price
                  </th>
                  <th style={{ borderBottom: '1px solid #000', fontSize: '10px', fontWeight: 'bold', textAlign: 'right' }}>
                    Total
              ...
              <tbody>
                {transaction.items.map((item, index) => (
                  <tr key={index}>
                    <td style={{ fontSize: '10px', maxWidth: '120px', wordWrap: 'break-word' }}>
                      {item.product.name}
                      <br />
            ...
            <div className="totals" style={{
              ...
              <div className="total-line" style={{
                ...
                <span>Payment ({transaction.payment_method.replace('_', ' ')}):</span>
